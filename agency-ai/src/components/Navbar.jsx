...
const Navbar = () => {
    ...
    return (
        <motion.div
            ...
            className='flex justify-between items-center px-4 sm:px-12 lg:px-24 xl:px-40 py-4 sticky top-0 z-20 backdrop-blur-xl font-medium bg-amber-50'>

            <img src={assets.logo} className='w-32 sm:w-40' alt=''/>

            <div
                className={`text-dark sm:text-sm ${!sidebarOpen ? 'max-sm:w-0 overflow-hidden' : 'max-sm:w-60 max-sm:pl-10'} max-sm:fixed top-0 bottom-0 right-0 max-sm:min-h-screen max-sm:h-full max-sm:flex-col max-sm:bg-primary max-sm:pt-20 flex sm:items-center gap-5 transition-all`}>
